...
function findUpPackageJsonDirectoryCached(cwd, directoryToPackage) {
    ...
    const packageRoot = findUpPackageJsonDirectoryCached(path_1.default.dirname(cwd), directoryToPackage);
    if (packageRoot) {
...
function resolveInlineModules(projectRoot, directoryToPackage, context, moduleName, platform) {
    ...
    if (inlineModuleFileExtension) {
        ...
        if (!modulePackageRoot) {
            modulePackageRoot = findUpPackageJsonDirectoryCached(path_1.default.dirname(context.originModulePath), directoryToPackage);
        }
        if (!modulePackageRoot) {
            return { type: 'empty' };
        }
