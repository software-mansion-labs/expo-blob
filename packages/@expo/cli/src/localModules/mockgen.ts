...
function getModuleTypes(
  ...
  return nestedClassDefinitions
    .flatMap((definition) => {
      ...
      const asyncFunctions = definition.asyncFunctions.map((asyncFunction) => {
        return ts.factory.createFunctionDeclaration(
          [ts.factory.createModifier(ts.SyntaxKind.AsyncKeyword)],
          undefined,
          asyncFunction.name,
          undefined,
          asyncFunction.types?.parameters.map((typeParam) =>
            ts.factory.createParameterDeclaration(
              undefined,
              undefined,
              typeParam.name,
              undefined,
              ts.factory.createTypeReferenceNode(typeParam.typename, undefined),
              undefined
            )
          ) ?? [],
          ts.factory.createTypeReferenceNode(asyncFunction.types?.returnType, undefined),
          undefined
        );
      });
      const propsType = generatePropTypesForDefinition(definition);
      ...
      return [propsType, defaultViewDeclaration, defaultViewExport, asyncFunctions];
    })
    .flat(1);
}
