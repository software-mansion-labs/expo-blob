...
export async function test({ describe, it, expect, jasmine }) {
    ...
    describe('Blob', async () => {
        ...
        describe('constructor', () => {
            it("globalThis should have a Blob property.", () => {expect("Blob" in globalThis).toBeTruthy()})
            it("Blob.length should be 0", () => {expect(Blob.length).toBe(0)})
            it("Blob should be a function", () => {expect(Blob instanceof Function).toBeTruthy()})
            ...
            it("ToString should be called on elements of the blobParts array and any exceptions should be propagated.", () => {
                expect(() => new Blob([{ toString: function() { throw test_error; } }])).toThrow(test_error)
                expect(() => new Blob([{ toString: undefined, valueOf: function() { throw test_error; } }])).toThrow(test_error)
                expect(() => new Blob([{
                    toString: function() { throw test_error; },
                    valueOf: function() { expect(false).toBe(true); }
                }])).toThrow(test_error)
            ...
            describe('Passing arguments for options', () => {
                [
                    ...
                    test_blob(function() {
                        ...
                        expected: "",
                        type: "",
                        desc: "Passing " + JSON.stringify(arg) + " (index " + idx + ") for options should use the defaults."
                    });
                    test_blob(function() {
                        return new Blob(["\na\r\nb\n\rc\r"], arg);
                    }, {
                        expected: "\na\r\nb\n\rc\r",
                        type: "",
                        desc: "Passing " + JSON.stringify(arg) + " (index " + idx + ") for options should use the defaults (with newlines)."
                    });
                });
            })
